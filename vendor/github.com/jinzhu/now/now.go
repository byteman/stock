...
func (now *Now) BeginningOfNDay(n int) time.Time {
	d := time.Duration(-now.Hour()) * time.Hour-(time.Hour*time.Duration(n*24))
	return now.BeginningOfHour().Add(d)
}
...
func (now *Now) EndOfNDay(n int) time.Time {
	return now.BeginningOfNDay(n).Add(24*time.Hour - time.Nanosecond)
}
